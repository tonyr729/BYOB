...
app.get('/api/v1/pictures/:id', (request, response) => {
  ...
  database('pictures').where('id', id).select()
    ...
    .catch(error => {
      response.status(500).json(
        { errorMessage: `Could not find picture with id of ${id}`, error }
      );        
    });
});
  ...
  database('games').where('id', picture.game_id).select()
    .then(game => {
      if (game.length) {
        ...
        response.status(500).json(
          { error: 'Could not find matching game for picture submitted' }
        );
      }
    })
    .catch(error => {
    ...
    .then(() => {
      database('games').where('id', id).del()
        ...
        .catch(() => response.status(404).json(
          { Error: `Cannot find matching game id: ${id}`}
        ));
    })
    .catch(error => {
      response.status(500).json({ error });
...
app.delete('/api/v1/pictures/:id', checkAuth, (request, response) => {
  ...
  database('pictures').where('id', id).del()
    .then(() => response.sendStatus(204))
    .catch(() => response.status(404).json(
      { Error: `Cannot find matching id: ${id}`}
    ));
});
